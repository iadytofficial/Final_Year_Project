...
export default function TransportRequests() {
  ...
  useEffect(()=>{(async()=>{const {data}=await api.get('/transport/trip-requests'); setItems(data?.items||data||[])})()},[])

  const respond = async (bookingId, accept) => {
    await api.put(`/transport/trip/${bookingId}/respond`, { accept })
  ...
  return (
    <ProtectedLayout>
      <div className="mx-auto max-w-5xl px-4 py-8">
        <h1 className="text-2xl font-semibold mb-4">Trip Requests</h1>
